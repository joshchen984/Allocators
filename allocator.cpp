...
Allocator::Allocator(std::uint32_t initial_capacity): heap{mmap(NULL, initial_capacity, PROT_READ | PROT_WRITE, MAP_PRIVATE | MAP_ANONYMOUS, -1, 0)} {
    if (heap == MAP_FAILED) {
        throw std::runtime_error("mmap failed");
    }
    free_list = new (heap) ChunkInfo{nullptr, nullptr, initial_capacity - sizeof(ChunkInfo), false};
}
...
void Allocator::free(void* ptr) {
    ...
    ChunkInfo* chunk = reinterpret_cast<ChunkInfo*>(static_cast<char*>(ptr) - sizeof(ChunkInfo));
...
void Allocator::splitChunk(Allocator::ChunkInfo* chunk, std::uint32_t size) {
    ...
    chunk->size = size;
    ChunkInfo* remainingChunk = new (nextChunkLocation(chunk)) ChunkInfo{chunk, chunk->next, remainingFreeSize, false};
    if(chunk->next) {
...
bool Allocator::should_split(std::uint32_t chunkSize, std::uint32_t allocateSize) {
    return chunkSize > allocateSize + sizeof(ChunkInfo);
}

Allocator::ChunkInfo* Allocator::next_free_chunk(Allocator::ChunkInfo* startChunk) {
...
Allocator::ChunkInfo* Allocator::next_free_chunk(Allocator::ChunkInfo* startChunk, std::uint32_t size) {
    ChunkInfo* cur = startChunk;
    while(cur != nullptr &&
            (cur->allocated || cur->size < size)) {
       cur = cur->next; 
    }
    return cur;
}
