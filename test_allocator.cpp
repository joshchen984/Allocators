...
TEST_CASE( "allocator", "[allocator]") {
    ...
    SECTION("Can allocate, write, and free one chunk of memory with more memory in allocator") {
    ...
    SECTION("Can allocate, write, and free one chunk of memory with no remaining memory in allocator") {
    ...
    SECTION("After freeing chunk, reuses freed memory when chunk uses all of memory") {
        void* ptr = a.allocate(170);
        fill_memory(ptr, 0x12, 170);
        check_pattern(ptr, 0x12, 170);
        a.free(ptr);
